...
public class FinalBossWeakAttack : BaseState
{
    ...
    public FinalBossWeakAttack (FinalBoss_FSM stateMachine) : base("weakattack", stateMachine)
    {
        _fbsm = stateMachine;
    }
    ...
    public override void Enter()
    {
        ...
        _fbsm.finalBossAnim.Play("weakattack");
    }
    ...
    public override void UpdateLogic()
    {
        base.UpdateLogic();
            ...
            if (_fbsm.finalBossAnim.GetCurrentAnimatorStateInfo(0).normalizedTime >= 0.4f) //if anim is at point of hitting player 
            {
                if (preventAttackingEveryFrame) //apply damage in if statement to prevent the damage from being applied every frame
                {
                    ...
                    foreach (Collider2D playerHit in playerToDamage) //for each enemy hit in the array declared above
                    {
                        ...
                        _fbsm.player.GetComponent<PlayerCombatManager>().TakeDamage(_fbcm.weakAttackDamage); //apply damage to player passing weak attack damage as parameter
        ...
        if (_fbsm.finalBossAnim.GetCurrentAnimatorStateInfo(0).normalizedTime >= 2f) //if anim finished
        {
            ...
            _fbsm.ChangeState(_fbsm.basicattack); //transition to moving state
        }
